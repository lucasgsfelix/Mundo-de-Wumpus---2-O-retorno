...
def processa_estado(probabilidade, posicao, angulo):
    ...
    if((coluna + 1) < 4):
        ...
        caminhos.append(
            [probabilidade[linha, coluna + 1], [linha, coluna + 1], 0])
    if((coluna - 1) >= 0):
        # print probabilidade[linha,coluna-1]
        # print "Deslocamento",[linha,coluna-1]
        caminhos.append([probabilidade[linha, coluna - 1],
                         [linha, coluna - 1], 180])
    if((linha + 1) < 4):
        # print probabilidade[linha+1,coluna]
        # print "Deslocamento",[linha+1,coluna]
        caminhos.append([probabilidade[linha + 1, coluna],
                         [linha + 1, coluna], 270])
    if((linha - 1) >= 0):
        # print probabilidade[linha-1,coluna]
        # print "Deslocamento",[linha-1,coluna]
        caminhos.append(
            [probabilidade[linha - 1, coluna], [linha - 1, coluna], 90])
    # print caminhos
    # print "---------------"
...
def geraMapaProbabilidades(tabelaProbabilidade):
    ...
    while(i < len(tabelaProbabilidade.values())):
        ...
        if(type(posicao[0]) == str) and (type(posicao[1]) == str):
            mapa[int(posicao[0])][int(posicao[1])
                                  ] = tabelaProbabilidade.values()[i]
        else:
